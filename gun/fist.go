package gun

import (
	"main/camera"
	"main/shaders"
	"main/utils"
	"math"

	"github.com/bob4321at/textures"
	"github.com/hajimehoshi/ebiten/v2"
)

type FistBullet struct {
	Img      *ebiten.Image
	Position utils.Vec2
	Vel      utils.Vec2
	Rotation float64
	Hit      bool
	Damage   int
	Remove   bool
}

func (bullet *FistBullet) Update() {}

func (bullet *FistBullet) Draw(screen *ebiten.Image) {}

func (bullet *FistBullet) Collide(position utils.Vec2, size utils.Vec2) bool {
	if utils.Collide(bullet.Position, utils.Vec2{X: float64(bullet.Img.Bounds().Dx()), Y: float64(bullet.Img.Bounds().Dy())}, position, size) {
		bullet.Remove = true

		return true
	}
	return false
}

func (bullet *FistBullet) GetDamage() int {
	return bullet.Damage
}

func (bullet *FistBullet) CheckRemoval() bool {
	return bullet.Remove
}

func CreateFistBullet(pos, vel utils.Vec2, rotation float64) *FistBullet {
	bullet := FistBullet{}

	bullet.Damage = 3

	bullet.Position = pos
	bullet.Vel = vel
	bullet.Rotation = rotation

	bullet.Img = ebiten.NewImage(16, 16)

	return &bullet
}

type FistAirBullet struct {
	Img      textures.RenderableTexture
	Position utils.Vec2
	Vel      utils.Vec2
	Rotation float64
	Hit      bool
	Damage   int
	Remove   bool
}

func (bullet *FistAirBullet) Update() {
	bullet.Position.X += bullet.Vel.X * 5
	bullet.Position.Y += bullet.Vel.Y * 5
}

func (bullet *FistAirBullet) Draw(screen *ebiten.Image) {
	op := ebiten.DrawImageOptions{}
	op.GeoM.Translate(-4, -4)
	op.GeoM.Rotate(utils.Deg2Rad(bullet.Rotation))
	op.GeoM.Translate(bullet.Position.X+camera.Camera.Pos.X, bullet.Position.Y+camera.Camera.Pos.Y)

	bullet.Img.Draw(screen, &op)
}

func (bullet *FistAirBullet) Collide(position utils.Vec2, size utils.Vec2) bool {
	if utils.Collide(bullet.Position, utils.Vec2{X: float64(bullet.Img.GetTexture().Bounds().Dx()), Y: float64(bullet.Img.GetTexture().Bounds().Dy())}, position, size) {
		bullet.Remove = true

		return true
	}
	return false
}

func (bullet *FistAirBullet) GetDamage() int {
	return bullet.Damage
}

func (bullet *FistAirBullet) CheckRemoval() bool {
	return bullet.Remove
}

func CreateFistAirBullet(pos, vel utils.Vec2, rotation float64) *FistAirBullet {
	bullet := FistAirBullet{}

	bullet.Damage = 2

	bullet.Position = pos
	bullet.Vel = vel
	bullet.Rotation = rotation

	bullet.Img = textures.NewTexture("./art/guns/handgun/wind.png", "")

	return &bullet
}

type FistGun struct {
	Bullets []Bullet
	Rot     float64

	Cooldown float64
	Charge   int

	Rendering_Rot float64
	Img           textures.RenderableTexture
}

func (gun *FistGun) Shoot(Charged int) {
	if gun.Cooldown < 0 {
		if Charged > gun.Charge {
			gun.Bullets = append(gun.Bullets, CreateFistAirBullet(utils.Vec2{X: Player_Pos.X + (640 / 2) + math.Cos(utils.Deg2Rad(gun.Rot))*15, Y: Player_Pos.Y + (360 / 2) + math.Sin(utils.Deg2Rad(gun.Rot))*15}, utils.Vec2{X: math.Cos(utils.Deg2Rad(gun.Rot)), Y: math.Sin(utils.Deg2Rad(gun.Rot))}, gun.Rot))
		} else {
			gun.Bullets = append(gun.Bullets, CreateFistBullet(utils.Vec2{X: Player_Pos.X + (640 / 2) + math.Cos(utils.Deg2Rad(gun.Rot))*15, Y: Player_Pos.Y + (360 / 2) + math.Sin(utils.Deg2Rad(gun.Rot))*15}, utils.Vec2{X: math.Cos(utils.Deg2Rad(gun.Rot)), Y: math.Sin(utils.Deg2Rad(gun.Rot))}, gun.Rot))
		}
		gun.Cooldown = 4
	}
}

func (gun *FistGun) Update() {
	if gun.Cooldown >= 0 {
		gun.Cooldown -= 0.1
	}

	gun.Rot = utils.Rad2Deg(-utils.GetAngle(utils.Vec2{X: 640 / 2, Y: 360 / 2}, utils.Vec2{X: utils.Mouse_X, Y: utils.Mouse_Y}) - 33)

	if 640/2 < utils.Mouse_X {
		gun.Rendering_Rot = utils.Rad2Deg(-utils.GetAngle(utils.Vec2{X: 640 / 2, Y: 360 / 2}, utils.Vec2{X: utils.Mouse_X, Y: utils.Mouse_Y}) - 33)
	} else {
		gun.Rendering_Rot = utils.Rad2Deg(utils.GetAngle(utils.Vec2{X: 640 / 2, Y: 360 / 2}, utils.Vec2{X: utils.Mouse_X, Y: utils.Mouse_Y}) - 33)
	}

	for bullet_index, bullet := range gun.Bullets {
		bullet.Update()
		for i := range Enemies_In_Level {
			enemy := Enemies_In_Level[i]
			if bullet.Collide(enemy.GetPosition(), enemy.GetSize()) {
				enemy.Hit(bullet.GetDamage())
			}

		}

		if bullet.CheckRemoval() {
			utils.RemoveArrayElement(bullet_index, &gun.Bullets)
			break
		}
	}
}

func (gun *FistGun) Draw(screen *ebiten.Image) {
	op := ebiten.DrawImageOptions{}
	op.GeoM.Translate(-24, -24)
	if 640/2 >= utils.Mouse_X {
		op.GeoM.Scale(-1, 1)
		op.GeoM.Rotate(utils.Deg2Rad(-gun.Rendering_Rot))
	} else {
		op.GeoM.Rotate(utils.Deg2Rad(gun.Rendering_Rot))
	}

	op.GeoM.Translate(640/2, 360/2)

	gun.Img.SetUniforms(map[string]any{
		"I": Player_I_Frames,
	})

	gun.Img.Draw(screen, &op)

	for i := range gun.Bullets {
		gun.Bullets[i].Draw(screen)
	}
}

func (gun *FistGun) GetImg() textures.RenderableTexture {
	return gun.Img
}

func (gun *FistGun) GetDroppedImg() textures.RenderableTexture {
	return gun.Img
}

func (gun *FistGun) GetCharge() int {
	return gun.Charge
}

func (gun *FistGun) CanShoot() bool {
	return gun.Cooldown <= 0
}

func CreateFistGun() Gun {
	gun := FistGun{}

	gun.Img = textures.NewTexture("./art/guns/handgun/gun.png", shaders.Flash_Shader)
	gun.Charge = 20

	return &gun
}
